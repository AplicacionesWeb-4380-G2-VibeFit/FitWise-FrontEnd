...
export default {
  ...
  methods: {
    ...
    getAllSchedules() {
      this.scheduleService.getAll().then(response => {
        ...
        this.userIds = [...new Set(rawScheds.map(sched => sched.userId))];
        // Obtain uniques healthPlanIds
        this.healthPlanIds = [...new Set(rawScheds.map(sched => sched.userId))];
  ...
  computed: {
    uniqueUserCount() {
      const ids = new Set(this.schedules.map(sched => sched.userId));
      return ids.size;
    },
    uniqueHealthPlanCount() {
      const ids = new Set(this.schedules.map(sched => sched.healthPlanId));
      return ids.size;
    }
  },
  created() {
